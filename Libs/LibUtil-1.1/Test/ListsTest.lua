...
describe("LibUtil[Lists]", function()
	...
	describe('LinkedList', function()
		...
		it("handles add at", function()
			...
			ll:AddAt(1, "TEST2")
			ll:AddAt(2, "TEST3")
			assert.same({"TEST1", "TEST2", "TEST3"}, ll:toTable())
			...
			ll:Add("TEST4")
			ll:AddFirst("TEST5")
			local index = ll:IndexOf("TEST2")
			ll:AddAt(index, "TEST2B")
			index = ll:IndexOf("TEST2")
			ll:AddAt(index + 1, "TEST2A")
			--index = ll:IndexOf("TEST4")
			--ll:AddAt(index, "TESTZ")
			--print(Util.Objects.ToString(ll:toTable()))
			assert.same({"TEST5", "TEST1", "TEST2B", "TEST2", "TEST2A", "TEST3", "TEST4"}, ll:toTable())
		end)

		it("handles insert (after)", function()
			local ll = LinkedList()
			assert.error(function() ll:InsertAfter("FOO", "TED") end)
			ll:Add("FOO")
			ll:Add("BAZ")
			--print(Util.Objects.ToString(ll:toTable()))
			assert.same({"FOO", "BAZ"}, ll:toTable())
			ll:InsertAfter("FOO", "TED")
			--print(Util.Objects.ToString(ll:toTable()))
			assert.same({"FOO", "TED", "BAZ"}, ll:toTable())
			ll:InsertAfter("TED", "RICE")
			--print(Util.Objects.ToString(ll:toTable()))
			assert.same({"FOO", "TED", "RICE", "BAZ"}, ll:toTable())
		end)
