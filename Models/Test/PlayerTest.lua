...
describe("Player", function()
    ...
    describe("functional", function()
        ...
        it("Get with invalid arguments", function()
            assert.has.errors(function() Player:Get() end, "'nil' (nil) is an invalid player")
            assert.has.errors(function() Player:Get(1) end, "'1' (number) is an invalid player")
        end)
        ...
        it("is comparable", function()
            ...
            local p3 = Player:Get('Player1')
            local p4 = Player:Get('Player2')
            assert.are.equal(p1, p1)
            assert.are.equal(p1, p3)
            assert.are.equal(p2, p2)
            assert.are.equal(p2, p4)
